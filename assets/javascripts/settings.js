...
function dialogAction() {
	...
	$("#pgconfig-dlg").dialog({
		...
		buttons: {
			"Ok": function () {
				var listBox = document.getElementById(pgListboxId);
				var fields = [
					"pg_config_name",
					"pg_currency",
					"pg_merchant_id",
				...
				var desc = values.join("|");
				var optText = values[0] + " | " + values[1] + " | " + values[2];
				opt.text = optText;
				opt.value = desc;
...
$("form").submit(function () {
	...
	var pglistbox = document.getElementById("settings_wktime_pg_config");
	if (pglistbox != null) {
		for (i = 0; i < pglistbox.options.length; i++) {
			pglistbox.options[i].selected = true;
		}
	}
...
// Payment Gateway Configuration Dialog Functions
function showPgConfigDialog(action, listId) {
	pgListboxId = listId;
	var listbox = document.getElementById(pgListboxId);
	var fields = ["pg_config_name", "pg_currency", "pg_merchant_id", "pg_work_key", "pg_access_code", "pg_redirect_url", "pg_cancel_url", "pg_url"];

	if (action === 'Add') {
		pgConfigAction = action;
		for (var i = 0; i < fields.length; i++) {
			document.getElementById(fields[i]).value = "";
		}
		$("#pgconfig-dlg").dialog("open");
	} else if (action === 'Edit' && listbox && listbox.options.selectedIndex >= 0) {
		pgConfigAction = action;
		var values = listbox.options[listbox.selectedIndex].value.split('|');
		for (var i = 0; i < fields.length; i++) {
			document.getElementById(fields[i]).value = values[i] || "";
		}
		$("#pgconfig-dlg").dialog("open");
	} else if (listbox && listbox.options.length > 0) {
		alert(selectListAlertMsg);
	}
}

function removeSelectedPgConfig(elementID) {
	var listbox = document.getElementById(elementID);
	if (listbox && listbox.selectedIndex >= 0) {
