  ...
  def getCFInRowHTML
	...
	def signAttendance
		...
		if params[:str] != "start" #&& isAccountUser
			...
			@lastAttnEntry.hours = computeWorkedHours(@lastAttnEntry.start_time,@lastAttnEntry.end_time)
			@lastAttnEntry.save()
		else
			addNewAttendance
		end	
	...
	def addNewAttendance
		...
		if params[:str] != "start"		
			...
			endtime = currentDate
			wkattendance.hours = computeWorkedHours(wkattendance.start_time,wkattendance.end_time) 
		else
	...
	def computeWorkedHours(startTime,endTime)
