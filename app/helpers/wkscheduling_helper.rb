...
module WkschedulingHelper
	...
	def link_to_wkcalendar_display_day(calendarObject, options={})
		...
		if !s.blank?
			...
			s =  "<div class='issue'>" + link_to(sprite_icon('add', ("Add Preference")), url_for(:controller => controller_name, :action => 'edit', :date => options[:day], :only_path => true), :class => 'icon icon-add') + "</div>".html_safe
		end
