...
Payroll module allow the user to view salary and User salary settings. T&E admin can manage other users 'salary components' and 'User salary settings'. A scheduled job will be run at start of each month or week or bi-week depending upon the pay period to generate the salary. T&E admin can manually Generate salary using 'Generate Salary' link.
