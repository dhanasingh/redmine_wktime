= ERPmine
= An upcoming Webinar on ERPmine Time & Expense on April 28 2021, 3PM IST (+5:30 GMT)

Please click here to register if interested 
https://us02web.zoom.us/meeting/register/tZcrcOCvqTouGNALBqMGF1GuxyKYjqAcesyM
...
Release Notes for v4.2.1
  ...
  Features:
    - Redmine 4.2 compatible
    - Approve expense entries on reimbursement

  Bug fixes:
    - Issue logger dropdown not loaded
    - Change Mapbox label
    - Showing old contract number in invoice
    - Reimbursement not showing for all project expense
    - Attachment download failed in time and expense
    - Show on Map option not visibile in HR
