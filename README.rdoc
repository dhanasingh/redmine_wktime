...
The following configurations can be made on the plugin
	...
	- Join date, Date of birth, Designation, EmployeeId can be configured
...
Attendance module allow the user to clock in, clock out and view their leave status using report.
T&E admin can manage other users attendance. A scheduler will be run at start of each month to calculate the opening balance.
...
Release Notes for 2.1
    ...
    Bugs:
        - Fixed SQL Server iisue.
		- Fixed time entering format issue.
		- Fixed issue forbidden error on "Enter issue as ID/Subject".

We would like to thank Luna Lenardi for contributing German Translation and bug fixing on currency.
