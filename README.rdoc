...
Release Notes for 2.0
	Thanks to Fotonation for sponsoring this release.
	Features:
		- Reminder email for submission and approval can be send by manager or TE admins.
		- TE admins are allowed to modify and approve time/expense.
		- User custom field as filters in list page.
		- "Please select" option is added in Issue and Activity dropdown.
		- In log time page, error message is shown for restricted tracker type and on TE related error, the Create/Save button is disabled.
