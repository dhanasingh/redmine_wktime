...
class WktimeHook < Redmine::Hook::ViewListener
	...
	def view_projects_show_sidebar_bottom(context={})
		if !context[:project].blank?
			...
			if wktime_helper.showExpense && (project_ids.blank? || (!project_ids.blank? && (project_ids == [""] || project_ids.include?("#{context[:project].id}"))) && User.current.allowed_to?(:view_time_entries, context[:project]))
